...
class TweetCrawler {
    ...
    cleanTweetText(text, isDetailMode, userMentions, quotedText) {
    ...
    async checkEndOfContent(page, collectedTweets, lastTweetCount, scrollsWithoutNewTweets, lastPageHeight, sameHeightCount, consecutiveTimeouts, searchFromDate) {
        const MAX_SCROLLS_WITHOUT_TWEETS = 8;
        const MAX_SAME_HEIGHT_COUNT = 10;
        // 1. Check end of timeline elements
        const endOfContentSelectors = (0, errorDetection_1.getAllEndOfContentPatterns)();
    ...
    async crawl(options) {
        const { searchKeywords, searchFromDate, searchToDate, targetCount = 10, delayEachTweetSeconds, delayEvery100TweetsSeconds, tweetThreadUrl, searchTab = "LATEST", searchUsernames = [], configType } = options;
        ...
        MODIFIED_SEARCH_KEYWORDS = MODIFIED_SEARCH_KEYWORDS ?
            ...
            "-filter:replies";
        const timeoutLimits = (0, CrawlConfig_1.getTimeoutLimits)(this.config, IS_UNLIMITED_SEARCH);
        ...
        try {
            ...
            // Go to search page
            await page.goto(IS_DETAIL_MODE ? tweetThreadUrl : constants_1.TWITTER_SEARCH_ADVANCED_URL[searchTab]);
            // Verify login
    ...
    async scrollAndCollectTweets(page, options) {
        const { TARGET_TWEET_COUNT, TIMEOUT_LIMIT, REACH_TIMEOUT_MAX, MAX_EXECUTION_TIME, startTime, IS_SEARCH_MODE, IS_DETAIL_MODE, DELAY_EACH_TWEET_SECONDS, DELAY_EVERY_100_TWEETS_SECONDS, SEARCH_FROM_DATE, MODIFIED_SEARCH_KEYWORDS, SEARCH_TAB } = options;
        let timeoutCount = 0;
        let reachTimeout = 0;
        ...
        const blacklistToken = (tokenIndex, reason) => {
        ...
        const rotateAuthToken = async (reason = "scheduled rotation") => {
        ...
        if (IS_SEARCH_MODE && MODIFIED_SEARCH_KEYWORDS) {
            ...
            const humanTypeText = async (element, text) => {
                await element.click();
        ...
        while ((TARGET_TWEET_COUNT === -1 || collectedTweets.length < TARGET_TWEET_COUNT) &&
            ...
            if (timeoutCount > TIMEOUT_LIMIT && reachTimeout < REACH_TIMEOUT_MAX) {
                reachTimeout++;
                timeoutCount = 0;
                await (0, scrollUtils_1.scrollUp)(page);
                await page.waitForTimeout(this.config.SCROLL.STABILIZATION_DELAY / 2);
                await (0, scrollUtils_1.scrollDown)(page);
            }
            ...
            if (response) {
                ...
                try {
                    ...
                    const errorDetection = (0, errorDetection_1.detectTokenError)(responseText);
                    if (errorDetection.hasError) {
                        ...
                        const waitTime = Math.min(this.config.RATE_LIMIT.BASE_WAIT_TIME * Math.pow(2, rateLimitRetries), this.config.RATE_LIMIT.MAX_WAIT_TIME);
                    ...
                    if (!tweets || !tweets.length) {
                        const isEndOfContent = await this.checkEndOfContent(page, collectedTweets, lastTweetCount, scrollsWithoutNewTweets, lastPageHeight, sameHeightCount, consecutiveTimeouts, SEARCH_FROM_DATE);
                        if (isEndOfContent) {
                    ...
                    const errorMessage = error instanceof Error ? error.message : String(error);
                    if (!(0, errorDetection_1.detectTokenErrorInMessage)(errorMessage)) {
...
//# sourceMappingURL=TweetCrawler.js.map